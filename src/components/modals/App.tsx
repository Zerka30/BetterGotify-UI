...
const AppModal: React.FC<AppModalProps> = ({ isOpen, onClose, onSave, mode, app }) => {
    ...
    const handleSubmit = async () => {
        // Validation de base
        if (!name.trim()) {
            setError(t('applications.errors.appNameRequired'));
            return;
        }
        ...
        try {
            ...
            setError(t('applications.errors.savingApplicationFailed'));
        } finally {
                    ...
                    <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-lg leading-6 font-medium text-gray-900" id="modal-title">
                                {mode === 'create' ? t('applications.modals.createApp.title') : t('applications.modals.updateApp.title')}
                            </h3>
                            <button
                                type="button"
                                onClick={onClose}
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                <label htmlFor="name" className="block text-sm font-medium text-gray-700">
                                    {t('applications.modals.form.name')}
                                </label>
                                <input
                                    type="text"
                                    id="name"
                            ...
                            <div>
                                <label htmlFor="description" className="block text-sm font-medium text-gray-700">
                                    {t('applications.modals.form.description')}
                                </label>
                                <textarea
                                    id="description"
                                    value={description}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                    {t('applications.modals.form.image')}
                                </label>
                                <div className="flex flex-col sm:flex-row items-start sm:items-center">
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        >
                                            {imagePreview || (app?.image && !deleteCurrentImage) ? t('applications.modals.form.imageUpdate') : t('applications.modals.form.imageAdd')}
                                        </label>
                                        <p className="mt-1 text-xs text-gray-500 text-center">
                                            {t('applications.modals.form.allowSize')}
                                        </p>
                                    </div>
                                </div>
                            </div>
